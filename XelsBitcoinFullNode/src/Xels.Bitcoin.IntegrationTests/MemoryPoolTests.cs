    ...
    public class MemoryPoolTests
    {
        ...
        public void AddToMempool()
        {
            using (NodeBuilder builder = NodeBuilder.Create())
            {
                ...
                xelsNodeSync.GenerateXels(105); // coinbase maturity = 100
                ...
                var block = xelsNodeSync.FullNode.BlockStoreManager().BlockRepository.GetAsync(xelsNodeSync.FullNode.Chain.GetBlock(4).HashBlock).Result;
        ...
        public void AddToMempoolTrxSpendingTwoOutputFromSameTrx()
        {
            using (NodeBuilder builder = NodeBuilder.Create())
            {
                ...
                parentTx.Sign(xelsNodeSync.MinerSecret, false);
                xelsNodeSync.Broadcast(parentTx);
                // wiat for the trx to enter the pool
                TestHelper.WaitLoop(() => xelsNodeSync.CreateRPCClient().GetRawMempool().Length == 1);
                // mine the transactions in the mempool
                xelsNodeSync.GenerateXels(1, xelsNodeSync.FullNode.MempoolManager().InfoAllAsync().Result.Select(s => s.Trx).ToList());
        ...
        public void MempoolReceiveFromManyNodes()
        {
            using (NodeBuilder builder = NodeBuilder.Create())
            {
                ...
                xelsNodeSync.SetDummyMinerSecret(new BitcoinSecret(new Key(), xelsNodeSync.FullNode.Network));
                xelsNodeSync.GenerateXels(201); // coinbase maturity = 100
                TestHelper.WaitLoop(() => xelsNodeSync.FullNode.ConsensusLoop().Tip.HashBlock == xelsNodeSync.FullNode.Chain.Tip.HashBlock);
                TestHelper.WaitLoop(() => xelsNodeSync.FullNode.HighestPersistedBlock().HashBlock == xelsNodeSync.FullNode.Chain.Tip.HashBlock);
        ...
        public void MempoolAddNodeWithOrphans()
        {
            using (NodeBuilder builder = NodeBuilder.Create())
            {
                ...
                xelsNodeSync.SetDummyMinerSecret(new BitcoinSecret(new Key(), xelsNodeSync.FullNode.Network));
                xelsNodeSync.GenerateXels(101); // coinbase maturity = 100
                TestHelper.WaitLoop(() => xelsNodeSync.FullNode.ConsensusLoop().Tip.HashBlock == xelsNodeSync.FullNode.Chain.Tip.HashBlock);
                ...
                var block = xelsNodeSync.FullNode.BlockStoreManager().BlockRepository.GetAsync(xelsNodeSync.FullNode.Chain.GetBlock(1).HashBlock).Result;
                var prevTrx = block.Transactions.First();
                var dest = new BitcoinSecret(new Key(), xelsNodeSync.FullNode.Network);
                ...
                txOrphan.Sign(key.GetBitcoinSecret(xelsNodeSync.FullNode.Network), false);

                // broadcast the orphan
                xelsNodeSync.Broadcast(txOrphan);
                TestHelper.WaitLoop(() => xelsNodeSync.FullNode.MempoolManager().Orphans.OrphansList().Count == 1);
                // broadcast the parent
                xelsNodeSync.Broadcast(tx);
                TestHelper.WaitLoop(() => xelsNodeSync.FullNode.MempoolManager().Orphans.OrphansList().Count == 0);
        ...
        public void MempoolSyncTransactions()
        {
            using (NodeBuilder builder = NodeBuilder.Create())
            {
                ...
                foreach (var node in xelsNodeSync.FullNode.ConnectionManager.ConnectedPeers) node.Behavior<MempoolBehavior>().NextInvSend = 0;
