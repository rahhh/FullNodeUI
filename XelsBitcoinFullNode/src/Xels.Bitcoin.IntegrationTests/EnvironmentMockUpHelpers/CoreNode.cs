...
using static Xels.Bitcoin.BlockPulling.BlockPuller;
...
namespace Xels.Bitcoin.IntegrationTests.EnvironmentMockUpHelpers
{
    public class CoreNode
    {
        ...
        public FullNode FullNode
        {
            get
            {
                if (this.runner is XelsBitcoinPosRunner)
                    return ((XelsBitcoinPosRunner)this.runner).FullNode;

                return ((XelsBitcoinPowRunner)this.runner).FullNode;
            }
        }
        ...
        {
            var fullNode = (this.runner as XelsBitcoinPowRunner).FullNode;
            var state = new MempoolValidationState(true);

            return fullNode.MempoolManager().Validator.AcceptToMemoryPool(state, trx).Result;
        }

        public List<uint256> GenerateXelsWithMiner(int blockCount)
        {
        ...
        [Obsolete("Please use GenerateXelsWithMiner instead.")]
        public Block[] GenerateXels(int blockCount, List<Transaction> passedTransactions = null, bool broadcast = true)
        {
            var fullNode = (this.runner as XelsBitcoinPowRunner).FullNode;
