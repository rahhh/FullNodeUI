...
namespace Xels.Bitcoin.IntegrationTests.EnvironmentMockUpHelpers
{
    ...
    public class XelsBitcoinPosRunner : INodeRunner
    {
        private Action<IFullNodeBuilder> callback;

        public XelsBitcoinPosRunner(Action<IFullNodeBuilder> callback = null)
        {
        ...
        public void Start(string dataDir)
        {
            NodeSettings nodeSettings = new NodeSettings(Network.XelsRegTest, ProtocolVersion.ALT_PROTOCOL_VERSION, args:new string[] { "-conf=xels.conf", "-datadir=" + dataDir }, loadConfiguration:false);
    ...
    public class XelsBitcoinPowRunner : INodeRunner
    {
        private Action<IFullNodeBuilder> callback;

        public XelsBitcoinPowRunner(Action<IFullNodeBuilder> callback = null) : base()
        {
            this.callback = callback;
        }
