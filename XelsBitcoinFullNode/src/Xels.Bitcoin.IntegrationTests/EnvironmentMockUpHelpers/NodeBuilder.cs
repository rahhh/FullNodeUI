...
namespace Xels.Bitcoin.IntegrationTests.EnvironmentMockUpHelpers
{
    ...
    public class NodeBuilder : IDisposable
    {
            ...
            for (int retry = 0; retry < 2; retry++)
            {
                ...
                if (tryKill)
                {
                    ...
                    foreach (var bitcoind in Process.GetProcessesByName("bitcoind"))
                        if (bitcoind.MainModule.FileName.Contains("Xels.Bitcoin.IntegrationTests"))
                            bitcoind.Kill();
        ...
        public CoreNode CreateXelsPowNode(bool start = false, Action<IFullNodeBuilder> callback = null)
        {
            string child = this.CreateNewEmptyFolder();
            var node = new CoreNode(child, new XelsBitcoinPowRunner(callback), this, Network.RegTest);
            this.Nodes.Add(node);
            if (start)
                node.Start();
            return node;
        ...
        public CoreNode CreateXelsPosNode(bool start = false, Action<IFullNodeBuilder> callback = null)
        {
            string child = this.CreateNewEmptyFolder();
            var node = new CoreNode(child, new XelsBitcoinPosRunner(callback), this, Network.RegTest, configfile: "xels.conf");
            this.Nodes.Add(node);
            if (start)
                node.Start();
            return node;
        ...
        public CoreNode CloneXelsNode(CoreNode cloneNode)
        {
            var node = new CoreNode(cloneNode.Folder, new XelsBitcoinPowRunner(), this, Network.RegTest, false);
            this.Nodes.Add(node);
