...
namespace Xels.Bitcoin.Configuration.Logging
{
    ...
    public static class LoggingConfiguration
    {
        ...
        private static void AddFilters(LogSettings settings = null, DataFolder dataFolder = null)
        {
            ...
            var defaultRule = new LoggingRule($"{nameof(Xels)}.{nameof(Bitcoin)}.*", NLog.LogLevel.Info, mainTarget);
            ...
            if (settings.DebugArgs.Any())
            {
                if (settings.DebugArgs[0] == "1")
                {
                    ...
                    defaultRule = new LoggingRule($"{nameof(Xels)}.{nameof(Bitcoin)}.*", NLog.LogLevel.Debug, mainTarget);
                }
                else
                {
