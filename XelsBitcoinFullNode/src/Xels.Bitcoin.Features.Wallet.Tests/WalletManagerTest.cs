...
namespace Xels.Bitcoin.Features.Wallet.Tests
{
    public class WalletManagerTest : LogsTestBase, IDisposable, IClassFixture<WalletFixture>
    {
        ...
        public void CreateWalletWithoutPassphraseOrMnemonicCreatesWalletUsingPassword()
        {
            ...
            var chain = new ConcurrentChain(Network.XelsMain);
            ...
            var walletManager = new WalletManager(this.LoggerFactory.Object, Network.XelsMain, chain, NodeSettings.Default(), new Mock<WalletSettings>().Object,
            ...
            Assert.Equal(Network.XelsMain, expectedWallet.Network);
            ...
            Assert.Equal(actualWallet.EncryptedSeed, mnemonic.DeriveExtKey(password).PrivateKey.GetEncryptedBitcoinSecret(password, Network.XelsMain).ToWif());
            Assert.Equal(expectedWallet.EncryptedSeed, mnemonic.DeriveExtKey(password).PrivateKey.GetEncryptedBitcoinSecret(password, Network.XelsMain).ToWif());
        }
        ...
        public void CreateWalletWithPasswordAndPassphraseCreatesWalletUsingPasswordAndPassphrase()
        {
            ...
            Assert.Equal(actualWallet.EncryptedSeed, mnemonic.DeriveExtKey(passphrase).PrivateKey.GetEncryptedBitcoinSecret(password, Network.XelsMain).ToWif());
            Assert.Equal(expectedWallet.EncryptedSeed, mnemonic.DeriveExtKey(passphrase).PrivateKey.GetEncryptedBitcoinSecret(password, Network.XelsMain).ToWif());
        }
        ...
        public void LoadWalletWithExistingWalletLoadsWalletOntoManager()
        {
            ...
            var walletManager = new WalletManager(this.LoggerFactory.Object, Network.XelsMain, new Mock<ConcurrentChain>().Object, NodeSettings.Default(), new Mock<WalletSettings>().Object,
                                                dataFolder, new Mock<IWalletFeePolicy>().Object, new Mock<IAsyncLoopFactory>().Object, new NodeLifetime(), DateTimeProvider.Default);
        ...
        public void GetSpendableTransactionsReturnsTransactionsGivenBlockHeight()
        {
            ...
            wallet.AccountsRoot.Add(new AccountRoot()
            {
                ...
                {
                    new HdAccount {
                        ExternalAddresses = WalletTestsHelpers.CreateUnspentTransactionsOfBlockHeights(Network.XelsMain, 8,9,10),
                        InternalAddresses = WalletTestsHelpers.CreateUnspentTransactionsOfBlockHeights(Network.XelsMain, 8,9,10)
                    }
                }
            });
            ...
            {
                ExternalAddresses = WalletTestsHelpers.CreateUnspentTransactionsOfBlockHeights(Network.XelsMain, 1, 3, 5, 7, 9, 10),
                InternalAddresses = WalletTestsHelpers.CreateUnspentTransactionsOfBlockHeights(Network.XelsMain, 2, 4, 6, 8, 9, 10)
            });
        ...
        {
            var result = HdOperations.CreateHdPath((int)CoinType.Xels, 4, 3, true);
        ...
        {
            var result = HdOperations.CreateHdPath((int)CoinType.Xels, 4, 3, false);
