...
namespace NBitcoin.Tests
{
        ...
        // Require a xels node running with the following xels.conf file
        //server=1
        //rpcuser=rpcuser
    ...
    public class NodeBuilderXels : IDisposable
    {
        public static NodeBuilderXels Create([CallerMemberName]string caller = null, string version = "0.12.1")
        {
            ...
            return new NodeBuilderXels(caller, path);
        }
        ...
        public NodeBuilderXels(string root, string bitcoindPath)
        {
            this._Root = root;
        ...
        private readonly List<CoreNodeXels> _Nodes = new List<CoreNodeXels>();
        public List<CoreNodeXels> Nodes
        {
            get
            {
                return _Nodes;
        ...
        public CoreNodeXels CreateNode(bool start = false)
        {
            ...
            //{
            //}
            var node = new CoreNodeXels(child, this);
            Nodes.Add(node);
            return node;
        }
        ...
        {
            if (!Process.GetProcesses().Any(p => p.ProcessName.Contains("xels")))
                throw new NotSupportedException("xels node is not running");
    ...
    public class CoreNodeXels
    {
        private readonly NodeBuilderXels _Builder;
        ...
        public RPCClient CreateRPCClient()
        {
            ...
            return new RPCClient(new NetworkCredential("rpcuser", "rpcpassword"), new Uri("http://127.0.0.1:" + Network.XelsMain.RPCPort + "/"), Network.XelsMain);
        }
        ...
        {
            return Node.Connect(Network.XelsMain, "127.0.0.1:" + Network.XelsMain.DefaultPort); //ports[0].ToString());
        }
